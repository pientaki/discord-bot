...
class Search(commands.Cog):
    ...
    async def weather(self, ctx: commands.Context, city: str):
        ...
        async with aiohttp.ClientSession() as cs:
            async with cs.get(complete_url) as response:
                ...
                if x["cod"] != "404":
                    async with ctx.typing():
                        ...
                        embed.add_field(name="気温", value=f"**{current_temperature_celsiuis}°C**")
                        embed.add_field(name="湿度", value=f"**{current_humidity}%**")
                        embed.add_field(name="気圧", value=f"**{current_pressure}hPa**")
